...
-- Tiempo de generación: 20-11-2018 a las 20:53:00
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_GESTION_ACTUALIZAR` (IN `idTrabajo` INT(11), IN `peso` DECIMAL(10,2), IN `lote` DECIMAL(10,2), IN `ordenTra` VARCHAR(100), IN `cant` INT(11), IN `idpersona` INT(11))  NO SQL
BEGIN 

UPDATE `ovillado` SET `PesoOvillo`=peso,`LoteOvillo`=lote,`NumOrden`=ordenTra,`Cantidadovillos`=cant,`Persona_idPersona`=idpersona WHERE `idOvillado`=idTrabajo;  
...
SELECT o.idOvillado,o.PesoOvillo,o.LoteOvillo,o.NumOrden as CodigoTrabajo,o.Cantidadovillos,o.Persona_idPersona,ma.Descripcion as NombreMaterial FROM ovillado o INNER JOIN material ma On ma.idMaterial=o.Material_idMaterial WHERE O.idOvillado=idOrDENe; 
END$$

DROP PROCEDURE IF EXISTS `SP_GESTION_REGISTRO`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_GESTION_REGISTRO` (IN `idOrdenR` INT(11), IN `pesoO` DECIMAL(10,2), IN `loteOv` VARCHAR(100), IN `NumTrabajo` VARCHAR(100), IN `idMaterialR` INT(11), IN `cantidadO` INT(11), IN `idPersonaR` INT(11))  NO SQL
BEGIN 


INSERT INTO `ovillado`(`idOvillado`, `Orden_idOrden`, `PesoOvillo`, `LoteOvillo`, `NumOrden`, `Material_idMaterial`, `Cantidadovillos`, `Persona_idPersona`, `fechaRegistro`, `Estado_idEstado`) VALUES (NULL,idOrdenR,pesoO,loteOv,NumTrabajo,idMaterialR,cantidadO,idPersonaR,NOW(),1);
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_ORDEN_ACTUALIZAR` (IN `material` INT(11), IN `loteE` VARCHAR(100), IN `kilosE` DECIMAL(10,2), IN `numConoE` DECIMAL(10,2), IN `creador` INT(11), IN `idOrdenE` INT(11))  NO SQL
BEGIN 

UPDATE `orden` SET `Material_idMaterial`=material,`Lote`=loteE,`Kilos`=kilosE,`NumConos`=numConoE WHERE `idOrden`=idOrdenE;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_ORDEN_REGISTRO` (IN `numOrdenR` VARCHAR(100), IN `idMaterialR` INT(11), IN `LoteR` VARCHAR(100), IN `KilosR` DECIMAL(10,2), IN `NumConos` DECIMAL(10,2), IN `creador` INT(11))  NO SQL
BEGIN 


INSERT INTO `orden`(`idOrden`, `NumOrden`, `Material_idMaterial`, `Lote`, `Kilos`, `NumConos`, `fechaRegistro`, `Estado_idEstado`) VALUES (NULL,numOrdenR,idMaterialR,LoteR,KilosR,NumConos,NOW(),10);
...
SELECT o.idOrden,o.NumOrden,ma.Descripcion,o.Lote,o.Kilos,o.NumConos,o.Estado_idEstado,e.nombreEstado,DATE_FORMAT(o.fechaRegistro,"%d/%m/%Y") as fechaRegistro FROM orden o INNER JOIN estado e ON e.idEstado=o.Estado_idEstado INNER JOIN material ma On ma.idMaterial=o.Material_idMaterial WHERE o.Estado_idEstado=9 or o.Estado_idEstado=8 or o.Estado_idEstado=7 or o.Estado_idEstado=6 or o.Estado_idEstado=5;
...
CONCAT(p.nombrePersona,' ',p.apellidoPaterno,' ',p.apellidoMaterno,' DNI: ',p.DNI) as Trabajador,
ov.PesoOvillo,
ov.LoteOvillo,
...
(3, 'PRIMA COTTON - 000234', '2018-11-08 00:00:00', 1),
(4, 'PRIMA ACRYLIC - 000235', '2018-11-08 00:00:00', 1),
(5, 'PRIMA WOOL - 000236', '2018-11-08 00:00:00', 1);
...
INSERT INTO `orden` (`idOrden`, `NumOrden`, `Material_idMaterial`, `Lote`, `Kilos`, `NumConos`, `fechaRegistro`, `Estado_idEstado`) VALUES
(1, 'Nº 0001', 5, 'AN1', '50.00', '13.50', '2018-11-20 15:06:37', 6),
(2, 'Nº 0002', 4, '12', '13.00', '12.00', '2018-11-20 15:34:47', 6),
(3, 'Nº 0003', 4, '12', '12.00', '12.00', '2018-11-20 15:37:01', 6);
...
INSERT INTO `ovillado` (`idOvillado`, `Orden_idOrden`, `PesoOvillo`, `LoteOvillo`, `NumOrden`, `Material_idMaterial`, `Cantidadovillos`, `Persona_idPersona`, `fechaRegistro`, `Estado_idEstado`) VALUES
(4, 1, '12.00', '12', 'OV - 0001', 5, 12, 40, '2018-11-20 15:31:14', 1),
(5, 2, '123.00', '312', 'OV - 0002', 4, 123, 48, '2018-11-20 15:36:11', 1),
(6, 3, '12.00', '12', 'OV - 0003', 4, 12, 1, '2018-11-20 15:38:12', 1);
