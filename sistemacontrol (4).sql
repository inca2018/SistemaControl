...
-- Tiempo de generación: 05-12-2018 a las 22:01:32
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_GENERAR_REPORTE` (IN `Inicio` DATE, IN `Fin` DATE)  NO SQL
BEGIN 
DECLARE done INT DEFAULT FALSE;
DECLARE v_idMaterial BIGINT;
DECLARE v_idNombreMaterial VARCHAR(150);

DECLARE diaTemporal DATE;


DECLARE CursorReporte CURSOR FOR 
SELECT ma.idMaterial,ma.Descripcion  FROM material ma;

DECLARE CONTINUE HANDLER FOR NOT FOUND SET done = TRUE;

CREATE TEMPORARY TABLE TablaTemporal (Material VARCHAR(150),Fecha DATE, 
TotalEntregado DECIMAL(10,2), 
TotalPedido DECIMAL(10,2),
Promedio1 DECIMAL(10,2), 
...
FETCH CursorReporte into v_idMaterial,v_idNombreMaterial;
...
SET @TotalGenerado=(SELECT SUM(ov.Cantidadovillos) FROM ovillado ov WHERE ov.Material_idMaterial=v_idMaterial and DATE_FORMAT(ov.fechaRegistro,'%Y-%m-%d')=diaTemporal);

SET @TotalSolicitado=(SELECT SUM(ord.PedidoOvillo) FROM orden ord WHERE ord.Material_idMaterial=v_idMaterial and DATE_FORMAT(ord.fechaRegistro,'%Y-%m-%d')=diaTemporal);

SET @Promedio1=(SELECT SUM(ov.Cantidadovillos) FROM ovillado ov WHERE DATE_FORMAT(ov.fechaRegistro,'%Y-%m-%d')=diaTemporal);

SET @Promedio2=(SELECT SUM(ord.PedidoOvillo) FROM orden ord WHERE DATE_FORMAT(ord.fechaRegistro,'%Y-%m-%d')=diaTemporal);

INSERT INTO TablaTemporal(Material,Fecha,TotalEntregado,TotalPedido,Promedio1,Promedio2) VALUES (v_idNombreMaterial,diaTemporal,@TotalGenerado,@TotalSolicitado,@Promedio1,@Promedio2); 
...
SELECT Material,Fecha,IFNULL(TotalEntregado,'0') as TotalEntregado,IFNULL(TotalPedido,'0') as TotalPedido,IFNULL(Promedio1,'0') as Promedio1,IFNULL(Promedio2,'0') as Promedio2 FROM TablaTemporal  ORDER BY Fecha ASC;
END$$

DROP PROCEDURE IF EXISTS `SP_GENERAR_REPORTE2`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_GENERAR_REPORTE2` (IN `Inicio` DATE, IN `Fin` DATE)  NO SQL
BEGIN 
DECLARE done INT DEFAULT FALSE;
DECLARE v_idPersona BIGINT;
DECLARE v_idNombrePersona VARCHAR(150);

DECLARE diaTemporal DATE;
DECLARE contador INT(11);

DECLARE CursorReporte CURSOR FOR 
SELECT p.idPersona,CONCAT(p.nombrePersona,' ', p.apellidoPaterno,' ',p.apellidoMaterno) as Nombre FROM usuario u INNER JOIN persona p ON p.idPersona=u.Persona_idPersona WHERE u.Perfil_idPerfil=8;

DECLARE CONTINUE HANDLER FOR NOT FOUND SET done = TRUE;

CREATE TEMPORARY TABLE TablaTemporal (Persona VARCHAR(150),Fecha DATE, 
ResultadoAlcanzado DECIMAL(10,2), 
ProduccionDia DECIMAL(10,2),
Total DECIMAL(10,2),                                      
Promedio1 DECIMAL(10,2), 
Promedio2 DECIMAL(10,2),
Pendiente DECIMAL(10,2),                                      
ResultadoAlcanzadoAnterior DECIMAL(10,2),                             TotalAnterior DECIMAL(10,2),
ProAnterior DECIMAL(10,2)                                        
);           
...
FETCH CursorReporte into v_idPersona,v_idNombrePersona;
...
SET @ResultadoAlcanzado=(SELECT SUM(ov.Cantidadovillos) FROM ovillado ov WHERE ov.Persona_idPersona=v_idPersona and DATE_FORMAT(ov.fechaRegistro,'%Y-%m-%d')=diaTemporal);

SET @ProduccionDiaria=(SELECT SUM(ov.ProduccionDia)  FROM ovillado ov WHERE ov.Persona_idPersona=v_idPersona and DATE_FORMAT(ov.fechaRegistro,'%Y-%m-%d')=diaTemporal);

SET @Promedio1=(SELECT SUM(ov.Cantidadovillos) FROM ovillado ov WHERE  DATE_FORMAT(ov.fechaRegistro,'%Y-%m-%d')=diaTemporal);

SET @Promedio2=(SELECT SUM(ov.ProduccionDia)  FROM ovillado ov WHERE  DATE_FORMAT(ov.fechaRegistro,'%Y-%m-%d')=diaTemporal);

INSERT INTO TablaTemporal(Persona,Fecha,ResultadoAlcanzado,ProduccionDia,Total,Promedio1,Promedio2,Pendiente,ResultadoAlcanzadoAnterior,TotalAnterior,ProAnterior) VALUES (v_idNombrePersona,diaTemporal,@ResultadoAlcanzado,@ProduccionDiaria,(@ProduccionDiaria+0),@Promedio1,@Promedio2,0,0,0,0); 
...
SET @ResultadoAlcanzadoAnterior=(SELECT SUM(ov.Cantidadovillos) FROM ovillado ov WHERE ov.Persona_idPersona=v_idPersona and DATE_FORMAT(ov.fechaRegistro,'%Y-%m-%d')=@DiaAnterior);

SET @ProduccionDiariaAnterior=(SELECT SUM(ov.ProduccionDia)  FROM ovillado ov WHERE ov.Persona_idPersona=v_idPersona and DATE_FORMAT(ov.fechaRegistro,'%Y-%m-%d')=@DiaAnterior);

SET @ProduccionDiariaAnterior=(SELECT SUM(ov.ProduccionDia)  FROM ovillado ov WHERE ov.Persona_idPersona=v_idPersona and DATE_FORMAT(ov.fechaRegistro,'%Y-%m-%d')=@DiaAnterior);


INSERT INTO TablaTemporal(Persona,Fecha,ResultadoAlcanzado,ProduccionDia,Total,Promedio1,Promedio2,Pendiente,ResultadoAlcanzadoAnterior,TotalAnterior,ProAnterior) VALUES (v_idNombrePersona,diaTemporal,@ResultadoAlcanzado,@ProduccionDiaria,(@ProduccionDiaria+0),@Promedio1,@Promedio2,0,@ResultadoAlcanzadoAnterior,@ProduccionDiariaAnterior,@ProduccionDiariaAnterior);   
...
SELECT IFNULL(Persona,'-') as Persona,IFNULL(Fecha,'-') as Fecha,IFNULL(ResultadoAlcanzado,'0') as ResultadoAlcanzado,IFNULL(ProduccionDia,'0') AS ProduccionDia,IFNULL(Promedio1,'0') Promedio1,IFNULL(Promedio2,'0') AS Promedio2,IFNULL(Pendiente,'0') as Pendiente,
IFNULL(ResultadoAlcanzadoAnterior,'0') as ResultadoAlcanzadoAnterior,
IFNULL(TotalAnterior,'0') as TotalAnterior,IFNULL(ProAnterior,'0') AS ProAnterior  FROM TablaTemporal ORDER BY Fecha ASC;
END$$

DROP PROCEDURE IF EXISTS `SP_GESTION_ACTUALIZAR`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_GESTION_ACTUALIZAR` (IN `idTrabajo` INT(11), IN `peso` DECIMAL(10,2), IN `lote` VARCHAR(100), IN `ordenTra` VARCHAR(100), IN `cant` INT(11), IN `idpersona` INT(11), IN `Obs` TEXT, IN `Produccion` INT(11))  NO SQL
BEGIN 
...
UPDATE `ovillado` SET `PesoOvillo`=peso,`LoteOvillo`=lote,`NumOrden`=ordenTra,`Cantidadovillos`=cant,`ProduccionDia`=Produccion,`Persona_idPersona`=idpersona,`Observaciones`=Obs WHERE `idOvillado`=idTrabajo;  
...
SELECT o.idOvillado,o.PesoOvillo,o.LoteOvillo,o.NumOrden as CodigoTrabajo,o.Cantidadovillos,o.ProduccionDia,o.Persona_idPersona,ma.Descripcion as NombreMaterial,o.Observaciones FROM ovillado o INNER JOIN material ma On ma.idMaterial=o.Material_idMaterial WHERE O.idOvillado=idOrDENe; 
END$$

DROP PROCEDURE IF EXISTS `SP_GESTION_REGISTRO`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_GESTION_REGISTRO` (IN `idOrdenR` INT(11), IN `pesoO` DECIMAL(10,2), IN `loteOv` VARCHAR(100), IN `NumTrabajo` VARCHAR(100), IN `idMaterialR` INT(11), IN `cantidadO` INT(11), IN `idPersonaR` INT(11), IN `Obs` TEXT, IN `Produccion` INT(11))  NO SQL
BEGIN 
...
INSERT INTO `ovillado`(`idOvillado`, `Orden_idOrden`, `PesoOvillo`, `LoteOvillo`, `NumOrden`, `Material_idMaterial`, `Cantidadovillos`,`ProduccionDia`, `Persona_idPersona`, `fechaRegistro`, `Estado_idEstado`,`Observaciones`) VALUES (NULL,idOrdenR,pesoO,loteOv,NumTrabajo,idMaterialR,cantidadO,Produccion,idPersonaR,NOW(),1,Obs);
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_MATERIAL_ACTUALIZAR` (IN `Descri` VARCHAR(100), IN `estado` INT(11), IN `idMaterialE` INT(11), IN `creador` INT(11), IN `Produccion` INT(11))  NO SQL
BEGIN 

UPDATE `material` SET  `Descripcion`=UPPER(Descri),`Produccion_Diaria`=Produccion ,`Estado_idEstado`=estado WHERE `idMaterial`=idMaterialE;  
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_MATERIAL_REGISTRO` (IN `nom` VARCHAR(100), IN `estado` INT(11), IN `creador` INT(11), IN `Produccion` INT(11))  NO SQL
BEGIN 

-- REGISTRAR TIPO DE TARJETA --
INSERT INTO `material`(`idMaterial`, `Descripcion`,`Produccion_Diaria`, `fechaRegistro`, `Estado_idEstado`) VALUES (NULL,UPPER(nom),Produccion,NOW(),estado);
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_ORDEN_ACTUALIZAR` (IN `material` INT(11), IN `loteE` VARCHAR(100), IN `kilosE` DECIMAL(10,2), IN `numConoE` DECIMAL(10,2), IN `creador` INT(11), IN `idOrdenE` INT(11), IN `Obs` TEXT, IN `Pedido` INT(11))  NO SQL
BEGIN 

IF(Obs="-1")then 
SET Obs=null;
end if;
UPDATE `orden` SET `Material_idMaterial`=material,`Lote`=loteE,`Kilos`=kilosE,`NumConos`=numConoE,`PedidoOvillo`=Pedido,`Observaciones`=Obs WHERE `idOrden`=idOrdenE;
...
SELECT o.idOrden,o.NumOrden,ma.Descripcion,o.Lote,o.Kilos,o.NumConos,o.PedidoOvillo,o.Estado_idEstado,e.nombreEstado,DATE_FORMAT(o.fechaRegistro,"%d/%m/%Y") as fechaRegistro,o.RechazoEnconado,DATE_FORMAT(o.fechaRechazoEnconado,"%d/%m/%Y") as fechaRechazo FROM orden o INNER JOIN estado e ON e.idEstado=o.Estado_idEstado INNER JOIN material ma On ma.idMaterial=o.Material_idMaterial WHERE o.Estado_idEstado=10 or o.Estado_idEstado=9 or o.Estado_idEstado=8 or o.Estado_idEstado=7 or o.Estado_idEstado=6 or o.Estado_idEstado=5 ORDER BY o.idOrden DESC;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_ORDEN_REGISTRO` (IN `numOrdenR` VARCHAR(100), IN `idMaterialR` INT(11), IN `LoteR` VARCHAR(100), IN `KilosR` DECIMAL(10,2), IN `NumConos` DECIMAL(10,2), IN `creador` INT(11), IN `Obs` TEXT, IN `Pedido` INT(11))  NO SQL
BEGIN 
...
INSERT INTO `orden`(`idOrden`, `NumOrden`, `Material_idMaterial`, `Lote`, `Kilos`, `NumConos`,`PedidoOvillo`, `fechaRegistro`, `Estado_idEstado`,`Observaciones`) VALUES (NULL,numOrdenR,idMaterialR,LoteR,KilosR,NumConos,Pedido,NOW(),10,Obs);
...
(158, 'admin', 'ACTUALIZACION', 'Material', 'SE ACTUALIZO MATERIAL:ALPACA - 000234', '2018-12-05 09:28:32'),
(159, 'admin', 'ACTUALIZACION', 'Material', 'SE ACTUALIZO MATERIAL:ALPACA - 000234', '2018-12-05 09:28:40'),
(160, 'ADMINISTRADOR GENERAL DEL SISTEMA', 'REGISTRO', 'USUARIO', 'SE REGISTRO EL USUARIO:ffernandezr', '2018-12-05 10:20:11'),
(161, 'ADMINISTRADOR GENERAL DEL SISTEMA', 'REGISTRO', 'USUARIO', 'SE REGISTRO EL USUARIO:jsanchezs', '2018-12-05 10:20:30');
...
CREATE TABLE IF NOT EXISTS `material` (
  ...
  `Produccion_Diaria` int(10) DEFAULT NULL,
...
INSERT INTO `material` (`idMaterial`, `Descripcion`, `Produccion_Diaria`, `fechaRegistro`, `Estado_idEstado`) VALUES
(3, 'ALPACA - 000234', 150, '2018-11-08 00:00:00', 1),
(4, 'BABY MICROFIBRA - 000235', 200, '2018-11-08 00:00:00', 1),
(5, 'COTTON WOOL - 000236', 220, '2018-11-08 00:00:00', 1);
...
CREATE TABLE IF NOT EXISTS `orden` (
  ...
  `PedidoOvillo` int(11) DEFAULT NULL,
  `fechaRegistro` datetime NOT NULL,
  `Estado_idEstado` int(11) NOT NULL,
  `NumDescarte` int(11) DEFAULT NULL,
...
INSERT INTO `orden` (`idOrden`, `NumOrden`, `Material_idMaterial`, `Lote`, `Kilos`, `NumConos`, `PedidoOvillo`, `fechaRegistro`, `Estado_idEstado`, `NumDescarte`, `FechaCierre`, `ObsCierre`, `Observaciones`, `RechazoEnconado`, `fechaRechazoEnconado`, `RechazoOvillado`, `fechaRechazoOvillado`) VALUES
(1, 'Nº 0001', 3, '1', '12.00', '12.00', 200, '2018-04-02 12:55:32', 5, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL),
(2, 'Nº 0002', 4, '2', '15.00', '15.00', 200, '2018-04-02 12:55:50', 5, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL),
(3, 'Nº 0003', 5, '3', '12.50', '12.50', 200, '2018-04-02 12:56:08', 5, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL),
(4, 'Nº 0004', 3, '4', '15.00', '15.00', 280, '2018-04-03 12:57:18', 5, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL),
(5, 'Nº 0005', 4, '5', '19.50', '19.50', 250, '2018-04-03 12:57:50', 5, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL),
(6, 'Nº 0006', 5, '6', '17.00', '17.00', 275, '2018-04-03 12:58:12', 5, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL),
(7, 'Nº 0007', 3, '7', '20.00', '20.00', 290, '2018-04-04 12:59:13', 5, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL),
(8, 'Nº 0008', 4, '8', '15.00', '15.00', 255, '2018-04-04 12:59:26', 5, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL),
(9, 'Nº 0009', 5, '9', '15.00', '15.00', 305, '2018-04-04 12:59:48', 5, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL);
...
CREATE TABLE IF NOT EXISTS `ovillado` (
  ...
  `ProduccionDia` int(10) DEFAULT NULL,
...
INSERT INTO `ovillado` (`idOvillado`, `Orden_idOrden`, `PesoOvillo`, `LoteOvillo`, `NumOrden`, `Material_idMaterial`, `Cantidadovillos`, `ProduccionDia`, `Persona_idPersona`, `fechaRegistro`, `Estado_idEstado`, `Observaciones`) VALUES
(2, 2, '15.00', '2', 'OV - 0002', 4, 150, 200, 43, '2018-04-02 14:20:38', 1, NULL),
(3, 3, '12.50', '3', 'OV - 0003', 5, 125, 200, 46, '2018-04-02 14:21:02', 1, NULL),
(4, 4, '15.00', '4', 'OV - 0004', 3, 190, 200, 45, '2018-04-03 14:21:33', 1, NULL),
(5, 5, '19.50', '5', 'OV - 0005', 4, 195, 200, 43, '2018-04-03 14:21:54', 1, NULL),
(6, 8, '15.00', '6', 'OV - 0006', 4, 150, 200, 43, '2018-04-04 14:22:49', 1, 'ger'),
(7, 9, '15.00', '7', 'OV - 0007', 5, 150, 200, 46, '2018-04-04 14:23:17', 1, 'ergerg'),
(8, 1, '12.00', '1', 'OV - 0007', 3, 120, 200, 45, '2018-04-02 14:24:22', 1, 'teh'),
(9, 6, '17.00', '8', 'OV - 0008', 5, 170, 200, 46, '2018-04-03 14:25:08', 1, 'rthrt'),
(10, 7, '20.00', '9', 'OV - 0009', 3, 200, 200, 45, '2018-04-04 14:26:12', 1, 'egrer');
...
(49, 'ffernandezr', '$2a$08$7QoiynjOGNhCFrTDmgl15uiMbuHZdF1lX7/4wTYLi3/BMDyjnEkNm', 8, 43, 1, '2018-12-05 10:20:11'),
(50, 'jsanchezs', '$2a$08$jCj37eKmDUWts7apnuqYguwgWbju.GcUE.Db7UYpH4FNAYEEJKdJW', 8, 46, 1, '2018-12-05 10:20:30');
